"""
Retention Analyzer v3.0 — VDNA 3.0 Port
CTR benchmarking, series funnel planning, next-video comment suggestions.
Ported from old pipeline's RetentionOptimizationAgent.
"""
import os, json
...
class RetentionAnalyzer:
    ...
    def plan_series_funnel(self, topic_title: str, num_parts: int = 3) -> dict:
        ...
        for i in range(1, num_parts + 1):
            if i == 1:
                hook = "Part 1: What happened and why it matters"
                cta = "Part 2 drops tomorrow — subscribe and hit the bell so you don't miss it."
            elif i < num_parts:
                ...
                cta = "That's the full story. If this helped you understand, subscribe for more Telugu news breakdowns."
            parts.append({
                "part_number": i,
                "title": f"{clean_title} — Part {i}",
                "hook": hook,
                "end_cta": cta,
                "target_duration_sec": 480 if i == 1 else 360,
                "thumbnail_variant": i,
            })
        return {
